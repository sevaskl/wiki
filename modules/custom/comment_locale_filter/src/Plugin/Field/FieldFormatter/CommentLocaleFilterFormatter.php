...
namespace Drupal\comment_locale_filter\Plugin\Field\FieldFormatter;
 ...
 *
 * @FieldFormatter(
 *   id = "comment_locale_filter",
 *   module = "comment_locale_filter_multilang",
 *   label = @Translation("Comment Locale Filter"),
 *   field_types = {
 *     "comment"
 *   },
 *   quickedit = {
 ...
 * )
 */
class CommentLocaleFilterFormatter extends CommentDefaultFormatter {
    ...
    if ($status != CommentItemInterface::HIDDEN && empty($entity->in_preview) &&
      ...
      if ($this->currentUser->hasPermission('access comments') || $this->currentUser->hasPermission('administer comments')) {
        ...
        if ($entity->get($field_name)->comment_count || $this->currentUser->hasPermission('access comments')) {
          ...
          $comments = $this->storage->loadThread($entity, $field_name, $mode,
            $comments_per_page, $this->getSetting('pager_id'));
          if ($comments) {
            ...
            $build['pager']['#route_parameters'] = $this->routeMatch->getRawParameters()
              ->all();
